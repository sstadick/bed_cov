...
use ivtools::{self, IvStore, Lapper as IndexedLapper, ScAIList};
...
trait Runnable {
    fn run_cov(&self, file_a: &str, file_b: &str) -> Result<(), Box<dyn Error>>;
}
...
impl Runnable for Library {
    ...
    fn run_cov(&self, file_a: &str, file_b: &str) -> Result<(), Box<dyn Error>> {
        match *self {
            Library::LapperLib => LapperLib::run_cov(file_a, file_b),
            // Library::IndexedLapperLib => IndexedLapperLib::run_cov(file_a, file_b),
            Library::CoitreesLib => CoitreesLib::run_cov(file_a, file_b),
            Library::IITreeLib => IITreeLib::run_cov(file_a, file_b),
            Library::ScAIListLib => ScAIListLib::run_cov(file_a, file_b),
        }
    }
}

fn main() -> Result<(), Box<dyn Error>> {
    // TODO: program should be a subcommand
    let matches = clap_app!(bed_cov =>
        ...
        (@arg program: -p --program +takes_value +required possible_values(&Program::variants()) "Which Program to run")
    ...
    let program = value_t!(matches.value_of("program"), Program).unwrap();
    let lib = value_t!(matches.value_of("library"), Library).unwrap();
    match program {
        Program::Coverage => lib.run_cov(file_a, file_b)?,
    };
    Ok(())
}

impl LapperLib {
    fn run_cov(file_a: &str, file_b: &str) -> Result<(), Box<dyn Error>> {
...
//             let start = start.parse::<u32>()?;
//             let stop = stop.parse::<u32>()?;
//             bed.get_mut(chr).unwrap().push(ivtools::Interval {
//                 start,
//                 stop,
//                 val: (),
//             });
...
//         let mut lappers = HashMap::new();
//         for (key, value) in bed.into_iter() {
//             lappers.insert(key, IndexedLapper::new(value));
//         }
//         // Iter over B and get the values as we go
//         let mut handle = grep_cli::stdout(termcolor::ColorChoice::Never);
//         let mut reader = bedlib::bed_reader::BufReader::open(file_b)?;
...
//                 let mut cov = 0;
//                 let mut n = 0;
//                 for iv in lapper.aug_find(st0, en0) {
//                     n += 1;
//                     let st1 = if iv.start > st0 { iv.start } else { st0 };
//                     let en1 = if iv.stop < en0 { iv.stop } else { en0 };
//                     if st1 > cov_en {
...
impl ScAIListLib {
    fn run_cov(file_a: &str, file_b: &str) -> Result<(), Box<dyn Error>> {
        ...
        let mut lappers = HashMap::new();
        for (key, value) in bed.into_iter() {
            lappers.insert(key, ScAIList::new(value));
        }
        // Iter over B and get the values as we go
        let mut handle = grep_cli::stdout(termcolor::ColorChoice::Never);
        let mut reader = bedlib::bed_reader::BufReader::open(file_b)?;
